...
% --- Pandoc Switch Logic ---
% Define a new conditional for Pandoc environments
\newif\ifpandoc
\pandocfalse % Default is false for standard LaTeX (PDF)

% Check if the document is being processed by Pandoc
% Pandoc defines \pandoc during parsing
\ifdefined\pandoc
\pandoctrue
\fi

% --- PDF-only PostScript commands ---
\ifpandoc
% Skip PostScript specials in HTML output to avoid "unexpected Tok" errors
\else
...
\ifpandoc
% Simplified macros for Pandoc to prevent table corruption in HTML
\newcommand{\ttext}[1]{#1}
\newcommand{\ttitle}[1]{#1}

\newcommand{\tlineB}[2]{#1 & #2 \\ \hline}
\newcommand{\tlinetitB}[2]{#1 & #2 \\ \hline \hline}
\else
...
\ifpandoc
\title{\luacom  \ v1.4.1 User Manual}
\else
\title{\luacom \ User Manual \\
{\small (Version 1.4.1)}}
\fi

\author{Vinicius Almendra \and Renato Cerqueira \and Fabio Mascarenhas \and
  The OneLuaPro Project Authors}

\begin{document}

\maketitle

\chapter*{Preface: Note on Code Examples and Modern Windows Compatibility}

Please note that the code examples provided in this documentation
(such as the \textit{Microsoft Calendar Control}
\texttt{MSCAL.Calendar} or the \textit{Internet Transfer Control}
\texttt{InetCtls.Inet}) reflect a historical state of Windows
development. These ActiveX components (\texttt{.ocx} files) were
standard in the late 1990s and early 2000s but are \textbf{no longer
  included} by default in modern operating systems such as
\textbf{Windows 7, 10, and 11}.

\section*{Architecture Incompatibility (32-bit vs. 64-bit)}
A critical hurdle for these legacy examples is the CPU
architecture. Most historical ActiveX controls are strictly
\textbf{32-bit (x86)} components. If you are running a \textbf{64-bit
  version of the Lua interpreter}, it cannot load these 32-bit COM
objects, even if the \texttt{.ocx} files are present and registered on
your system. To use such legacy controls, a 32-bit Lua environment is
required.

\section*{Use of Examples}
These examples are intended primarily to demonstrate the
\textbf{LuaCOM syntax} and its core mechanics, specifically:
\begin{itemize}
\item The distinction between method calls using the colon operator
  (\texttt{:}) and property access using the dot operator
  (\texttt{.}).
\item The implementation of COM interfaces and event handling within
  Lua.
\item The use of \texttt{get}/\texttt{set} prefixes for property
  accessors.
\end{itemize}

\section*{Modern Alternatives}
For productive applications, it is strongly recommended to use current
alternatives that are natively available or 64-bit compatible:
\begin{itemize}
\item Instead of \texttt{InetCtls.Inet}, use the native
  \textbf{\texttt{WinHttp.WinHttpRequest.5.1}} object or libraries
  like \textit{LuaCurl}.
\item Instead of \texttt{MSCAL.Calendar}, use Lua's built-in
  \texttt{os.date} and \texttt{os.time} functions or modern UI
  frameworks.
\end{itemize}

The underlying logic and API of LuaCOM remain valid and function as
described, even if the specific objects used in the examples are no
longer available or compatible with your current system architecture.
...
\subsection{Newbies} For those who are newcomers, we provide a tutorial
...
\subsection{Knowledge required} This manual presumes some knowledge of
...
\subsection{Some information about samples} The sample codes shown in
...
Set the \verb+DateFormat+ field to \verb+"string_ms_accurate"+ to
format the value as a locale-specific string with correct rounding at
the half-second boundary. This option does not change the default format.
...
On Lua 5.3 and later, \ident{VT_I8} and \ident{VT_UI8} values use the
Lua integer subtype when the value is in its range. Larger unsigned
values use a Lua number and can lose precision.
...
\begin{itemize}
\item This \luacom\ version formally supports Lua 5.5.
\end{itemize}
...
\begin{verbatim}
-- outdated code, requires ActiveX-Controls like MSINET.OCX
inet_obj = luacom.CreateObject("InetCtls.Inet")
if inet_obj == nil then
  print("Error! Object could not be created!")
end

-- new code for Windows 10 and above
local luacom = require("luacom")
local http = luacom.CreateObject("WinHttp.WinHttpRequest.5.1")
http:Open("GET", "https://google.com", false)
http:Send()
