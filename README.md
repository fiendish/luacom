...
## COM lifetime and conversion behavior

LuaCOM keeps COM objects alive during callbacks, tracks event subscriptions, checks metadata, and releases partial conversion results on failure. It also preserves exact 64-bit integer values and routes `RoundTrip` conversion errors through `luacom.config.abort_on_API_error` and `luacom.config.last_error`.
